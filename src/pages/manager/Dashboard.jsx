...
import { Users, Briefcase, DollarSign, AlertTriangle, TrendingUp, UserCheck, PiggyBank, TrendingDown, Landmark, UserPlus, CalendarClock, FileQuestion, Loader2, Banknote, CalendarCheck, Wallet } from 'lucide-react';
...
const BranchManagerDashboard = () => {
  ...
  const fetchDashboardData = useCallback(async (start, end) => {
      ...
      try {
          ...
          const { data: statsData, error: statsError } = await supabase.rpc('get_branch_stats', params);
  ...
  const goMetricDrill = (metricKey) => {
    ...
    navigate(`/manager/dashboard/metric/${metricKey}?${q.toString()}`);
  };

  const statCards = [
    { title: 'Branch Borrowers', value: stats.total_borrowers, icon: Users, gradient: 'bg-gradient-to-br from-pink-500 to-rose-600', metric: 'borrowers' },
    { title: 'Branch Active Loans', value: stats.active_loans, icon: Briefcase, gradient: 'bg-gradient-to-br from-violet-500 to-purple-600', metric: 'active-loans' },
    { title: 'Branch Portfolio', value: formatCurrency(stats.total_portfolio), icon: PiggyBank, gradient: 'bg-gradient-to-br from-emerald-600 to-green-800', metric: 'portfolio' },
    ...
    { title: 'Loan Officers', value: stats.total_loan_officers || 0, icon: UserCheck, gradient: 'bg-gradient-to-br from-fuchsia-500 to-pink-600', metric: 'loan-officers' },
  ];

  const quickActions = [
    { title: 'Manage Loan Officers', icon: UserPlus, description: 'Add, view, and manage officers', path: '/manager/loan-officers' },
    { title: 'View Loan Requests', icon: FileQuestion, description: 'Approve loan edit/delete requests', path: '/manager/loan-requests' },
    { title: 'View Defaulters', icon: AlertTriangle, description: 'Track and manage loans in arrears', path: '/defaulters' },
  ];
