-- Add scheduled_repayments_collected to get_reports_metrics summary (cash by actual_payment_date).
  ...
  RETURN (
    ...
    summary AS (
      SELECT
        ...
        COALESCE((
          SELECT SUM(GREATEST(0, COALESCE(sr.amount, 0) - COALESCE(sr.prepay, 0)))
          FROM scoped_repayments sr
        ), 0) AS scheduled_repayments_collected,
        COALESCE((
    ...
    SELECT jsonb_build_object(
      'summary', (
        SELECT jsonb_build_object(
          ...
          'scheduled_repayments_collected', s.scheduled_repayments_collected,
...
COMMENT ON FUNCTION public.get_reports_metrics(date, date, uuid, uuid, uuid, uuid, uuid, text, text) IS
  'Reports metrics by actual_payment_date: scheduled vs prepayment split on day cash was received. Respects auth role scope.';
